...
6．収集した情報（エビデンス）をもとに検証するコンテンツの事象が科学的，論理的に正しいかを判断して，科学的判断が可能か難しいかをjudge_possible_science: boolで，論理的に判断が可能か難しいかをjudge_possible_logicでそれぞれtrue or falseで示したうえで，信憑性をtrue_percent: intで%表示してください．
    ...
    // responseを構造体に変換
    let res_json: fake_check_response = parse_response(&response.message().content);
...
//     description: String,
// }
fn parse_response(response: &str) -> fake_check_response {
    ...
    let judge_possible_science_str = &response[judge_possible_science_index..judge_possible_logic_index];
    ...
    let true_percent = true_percent_str.chars().filter(|c| c.is_digit(10)).collect::<String>().parse::<i32>().unwrap();
...
fn test_parse_response() {
    ...
    eprintln!("{:?}", &res_json);
    assert_eq!(res_json.judge_possible_science, false);
    assert_eq!(res_json.judge_possible_logic, true);
